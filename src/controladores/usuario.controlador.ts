...
import { CredenciaisInvalidasError, UsuarioEmailJaCadastradoError, UsuarioIDNaoEncontradoError } from "../erros/usuario.error";
...
class UsuarioControlador {
    ...
    logarUsuario = async (req: Request, res: Response, next: NextFunction) => {
        // Validando os dados da requisição
        ...
        // Obtendo os campos do body
        const { email, senha } = req.body;

        try {
            // Obtendo o usuário pelo e-mail
            const usuarioEncontrado = await this.usuarioRepositorio.obterUsuarioPeloEmail(email);

            // Se não for encontrado retornar um erro
            if (!usuarioEncontrado) {
                next(new CredenciaisInvalidasError());
                return;
            }

            // Verificando se a senha é válida
            if (!this.compararSenha(senha, usuarioEncontrado)) {
                next(new CredenciaisInvalidasError());
                return;
            }

            // Gerando o token
            const token = this.gerarTokenUsuario(usuarioEncontrado);
    ...
    private compararSenha = (senha: string, usuario: Usuario) => {
    ...
    private gerarTokenUsuario = (usuario: Usuario) => {
        const TOKEN_SECRET_KEY = process.env.TOKEN_SECRET_KEY || "";

        return jwt.sign({ sub: usuario.id }, TOKEN_SECRET_KEY, { expiresIn: "5h" });
    }
}
