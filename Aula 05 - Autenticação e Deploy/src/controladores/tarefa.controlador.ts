...
import { UsuarioIDNaoEncontradoError } from "../erros/usuario.error";
...
class TarefaControlador {
    ...
    obterTarefas = async (req: Request, res: Response, next: NextFunction) => {
        // Obtendo o id do usuário através do middleware
        const usuarioId = res.locals.id;

        try {
            // Verificando se o usuário existe
            const usuario = await this.usuarioRepositorio.obterUsuarioPeloID(usuarioId);
            ...
            const tarefas = await this.tarefaRepositorio.obterTarefasDoUsuario(usuario);
